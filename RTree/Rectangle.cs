    ...
    public class Rectangle
    {
        ...
        internal float distance(Point p)
        {
            ...
            for (int i = 0; i < DIMENSIONS; i++)
            {
                ...
                if (greatestMin > leastMax) distanceSquared += (greatestMin - leastMax) * (greatestMin - leastMax);
            }
...
#warning possible didn't convert properly
        ...
        internal void add(Rectangle r)
        {
            for (int i = 0; i < DIMENSIONS; i++)
            {
                if (r.min[i] < min[i]) min[i] = r.min[i];
                if (r.max[i] > max[i]) max[i] = r.max[i];
            }
        }
...
#warning possible didn't convert properly
                if (CompareArrays(r.min, min) && CompareArrays(r.max, max)) equals = true;
            }
