...
namespace RTree
{
    ...
    public class Node<T>
    {
        ...
        internal void deleteEntry(int i, int minNodeEntries)
        {
            ...
            // eliminated anyway.
            if (entryCount >= minNodeEntries) recalculateMBR(deletedRectangle);
        }
        ...
        internal void recalculateMBR(Rectangle deletedRectangle)
        {
            if (mbr.edgeOverlaps(deletedRectangle))
            {
                ...
                for (int i = 1; i < entryCount; i++) mbr.add(entries[i]);
            }
        }
        ...
        {
            if (index < entryCount) return entries[index];
            return null;
        }

        public int getId(int index)
        {
            if (index < entryCount) return ids[index];
            return -1;
        }
        ...
        internal void reorganize(RTree<T> rtree)
        {
            ...
            for (int index = 0; index < entryCount; index++)
                ...
                {
                    while (entries[countdownIndex] == null && countdownIndex > index) countdownIndex--;
